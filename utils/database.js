...
export const connectToDB = async () =>{ // Asynchronous function to connect to the database
    mongoose.set('strictQuery', true); // Enable strict mode for query filters

    if(isConnected){ // Check if the database is already connected
    ...
    } catch (error) { // Catch block to handle any errors during connection
